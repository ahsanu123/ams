    ...
    TEntity: EntityTrait,
{
    ...
    async fn get_all(&mut self) -> Result<Vec<TEntity::Model>, DbErr>;

    async fn get_by_id(
        &mut self,
        id: Self::PrimaryKeyType,
    ) -> Result<Option<TEntity::Model>, DbErr>;

    async fn update_by_model(
        &mut self,
        model: TEntity::ActiveModel,
    ) -> Result<TEntity::Model, DbErr>;

    async fn create(&mut self, model: TEntity::ActiveModel) -> Result<TEntity::Model, DbErr>;
