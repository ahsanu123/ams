use sea_query::{
    ...
    QueryBuilder, QueryStatementBuilder, QueryStatementWriter, SchemaStatementBuilder,
...
pub struct SqlConnectionProvider {
    pub db_pool: Pool<impl Database>,
    pub sql_connection: SqlConnection,
}

impl SqlConnectionProvider {
    pub async fn create_connection(&self, sql_connection: SqlConnection) -> Pool<impl Database> {
...
impl SqlxHelper for SqlConnectionProvider {
    ...
    async fn migrate_table(&self, table_statement: impl SchemaStatementBuilder) {
        let sql = table_statement.build(self.get_builder());
