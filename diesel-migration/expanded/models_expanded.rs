...
pub mod models {
    ...
    const _: () = {
            ...
            fn values(
                ...
                >::values((
                    std::option::Option::Some(
                        diesel::ExpressionMethods::eq(posts::title, self.title),
                    ),
                    std::option::Option::Some(
                        diesel::ExpressionMethods::eq(posts::body, self.body),
                    ),
                ))
            }
        }
        #[allow(unused_qualifications)]
        impl<'a, 'insert> diesel::insertable::Insertable<posts::table>
        for &'insert NewPost<'a> {
            type Values = <(
                std::option::Option<diesel::dsl::Eq<posts::title, &'insert &'a str>>,
                std::option::Option<diesel::dsl::Eq<posts::body, &'insert &'a str>>,
            ) as diesel::insertable::Insertable<posts::table>>::Values;
        ...
        impl<
            'a,
        > diesel::internal::derives::insertable::UndecoratedInsertRecord<posts::table>
        for NewPost<'a> {}
    };
}
